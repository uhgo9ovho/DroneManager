...
export default {
  ...
  methods: {
    ...
    getTimeRangeTimestamps(period) {
      const now = new Date();
      const nowMillis = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999).getTime();
      const zeroMillis = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
      ...
      const tempDate = new Date(zeroMillis); // 创建一个基于当前毫秒级时间戳的Date对象
      switch (period) {
        case "week":
